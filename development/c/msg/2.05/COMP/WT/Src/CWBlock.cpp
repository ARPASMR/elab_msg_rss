...
//
//  $Rma: CWBlock.cpp,v $
//  Revision 1.7  2009/06/24 16:17:00 espanyol
//  Decrease by one the first accessed element of the temporary vector m_Tmp
//  in the inverse functions so that it does not try to access any reference
//  outside the vector boundaries.
//
...
void CWBlock::St1DH_Inv (const unsigned int i_I, const unsigned int i_S)
{
	...
	if (sd2 > 1)
	{
		int *pB = &m_Tmp[i_S-1]; // Revision 1.7 
		unsigned int n;
		for (n=sd2 ; n ; n--)
		{
			const int h0 = *--pH;
			const int c0 = *--pL + ((h0 + 1) >> 1);
			if (n=sd2) // Revision 1.7 
			{
				*pB = c0 - h0;
				*--pB = c0;
			}
