...
//INTERFACE:
	...
	const
	{
		...
		for (unsigned int j=i_goodCol ; j<8 ; j++)
			for (unsigned int i=0 ; i<i_goodLine ; i++) t_blk.Cset (i, j, t_blk.Cget (i, tmp));
		
		// 2° Copying lines
		tmp = i_goodLine - 1;
		for (unsigned int i=i_goodLine ; i<8 ; i++)
			for (unsigned int j=0 ; j<8 ; j++) t_blk.Cset (i, j, t_blk.Cget (tmp, j));
		COMP_CATCHTHIS
	}
...
public:
	...
	)
	{
		...
		if (stop_line > m_NL) stop_line = m_NL;
		if (stop_column > m_NC)
		{
			stop_column = m_NC;
	...
	const
	{
		...
		// 2° Padding, if needed
		if (padding) pad_block (o_blk, real_columns, real_lines);
		COMP_CATCHTHIS
	}
...
#ifdef _DEBUG
		if (x_from != m_index) std::cerr << "!!! x_from != m_index\n";
		Assert (x_from == m_index, Util::CParamException ());
		Assert (x_to <= m_size, Util::CParamException ());
#endif
		for (unsigned long i=x_from ; i<x_to ; i++) m_data[i] = 0;
		m_index = x_to;
		COMP_CATCHTHIS
	}
...
#endif
		for (m_index=i_from ; m_index<=i_to ;) m_data[m_index++] = 0;
		COMP_CATCHTHIS
	}
