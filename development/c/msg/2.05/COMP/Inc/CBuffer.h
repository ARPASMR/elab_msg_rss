...
// INTERFACES:
	...
	inline void get_next_byte()
	{
		...
		{
			m_byte_IN = 0;
			if ((m_size + 4) <= m_byte_index) m_reached_end = true;
			return;
		}
		m_byte_IN = m_ptr[m_byte_index];
		if (check_marker)
			if (m_byte_IN == 0)
				...
				else m_byte_IN = m_ptr[m_byte_index];
			else
				// We have detected a marker pattern
				if (m_marker_position >= 0) m_marker_offset = 24 - m_marker_position;
				else m_marker_position = 24;
...
// INTERFACES:
	...
	)
	{
		...
		// fetch the buffer
		while (m_bit_index <= 24) get_next_byte ();
		COMP_CATCHTHIS_SPEED
	}
	...
	)
	{
		...
		m_bit_index -= i_nbBits;
		// fetch the buffer
		while (m_bit_index <= 24) get_real_next_byte ();
		COMP_CATCHTHIS
	}
...
// INTERFACES:
	...
	)
	{
		...
		if (++m_byte_index >= m_size) double_size ();
	...
	)
	{
		...
		if (i_byte == 0xFFU) put_real_next_byte (0x00U);
		COMP_CATCHTHIS_SPEED
	}
	...
	)
	{
		COMP_TRYTHIS_SPEED
		m_ptr[++m_byte_index] = i_byte;
		if (i_byte == 0xFFU) m_ptr[++m_byte_index] = 0;
		COMP_CATCHTHIS_SPEED
	}
