...
/* Get the selected text (= filename) */
  gtk_clist_get_text(GTK_CLIST(clist), row, col, &fn);
  switch(event->type)
  {
...
/* Dir-part */
  ...
  wl=Create_Clist(LAB_DIRLIST, selected_dir ,NULL,NULL,1,"Directories",80,NULL);
  wlx=SPack("Directory","v",wfl,"2",wl->parent,"ef2",NULL);

/* File-part */
  wfr=Create_Entry(LAB_FILF,but_func,"%-20s",if_filter);
  wr=Create_Clist(LAB_FILELIST,selected_file,NULL,NULL,1,"Files",80,NULL);
...
/* Create initial dir. content */
  ...
  #if __GTK_WIN32__ == 1
  {
    ...
    discmem_ready=FALSE;
    Add_Options(file_window,LAB_DISK,disk_func,2,disc,ndisc,10);
    discmem_ready=TRUE;
