...
/*************************************
 * Some functions to translate gtk2.0 funcs into gtk1.2 funcs.
 * Remove these if compiled for gtk2.0.
 * (Under construction!!!)
 *************************************/
#ifndef __GTK_20__
...
/* === CHECK OPTION MENU ITEMS === */
      ...
      if ((id) && (GTK_IS_COMBO_BOX(cur_wdgt)) && (!strcmp(id, cur_wdgt_id)))
        break;
#else
      if ((id) && (GTK_IS_OPTION_MENU(cur_wdgt)) && (!strcmp(id, cur_wdgt_id)))
        break;
#endif
    }
  }
 ...
 * Return: The rcstyle.
 *************************************/
GtkRcStyle *Create_RcStyle(GtkRcFlags flag,    /*GTK_RC_xx, xx=FG,BG,TEXT,BASE*/
...
{
  GtkRcStyle *rc_style=Create_RcStyle(flag,normal,prelight,active,
...
#else
        else if (GTK_IS_OPTION_MENU(wdgt))
#endif
        {
...
#else
      else if (GTK_IS_OPTION_MENU(cur_wdgt))
        gs=Create_Guistate(gs,wndtitle,cur_wdgt_id,"%d",
                  option_menu_get_history(cur_wdgt));
#endif
