...
const double  R_POL      =  6356.5838;  /* radius from Earth centre to pol             */
      double  SUB_LON    =     0.0;     /* longitude of sub-satellite point in radiant */
...
int main(int argc, char *argv[] ){
  ...
  if ( argc != 10 ){
    fprintf( stderr, "   usage: navig <SAT> <CH> <LATMIN> <LATMAX>  <LONMIN> <LONMAX> <RES> <FRAW> <FOUT>\n");
    fprintf( stderr, "   <SAT> :	MSG1 or MSG2\n");
    fprintf( stderr, "   <CH> :	L (for low resolution) or H (for HRV)\n");
    fprintf( stderr, "   <LATMIN> :	LAT MIN in dd.mmmm\n");
    fprintf( stderr, "   <LATMAX> :	LAT MAX in dd.mmmm\n");
    fprintf( stderr, "   <LONMIN> :	LON MIN in dd.mmmm\n");
    fprintf( stderr, "   <LONMAX> :	LON MAX in dd.mmmm\n");
    fprintf( stderr, "   <RES> :	resolution in dd.mmmm\n");
    fprintf( stderr, "   <FRAW> :	file name raw MSG produced by xpic2raw\n");
    fprintf( stderr, "   <FOUT> :	file name in output\n");
    fprintf( stderr, "\n   call of program %d\n", argc);
    fprintf( stderr, "   EXIT!\n\n");
    exit (-1);
  }
...
/* Gestione rapid scan */
  ...
  strncpy(sat, argv[1] ,4);

/*
  if (strcmp(sat,"MSG1")==0)
	SUB_LON = 0.0 * PI / 180.;
  else if (strcmp(sat,"MSG2")==0)
	SUB_LON = 0.0;
  else {
    fprintf( stderr, " Unknown satellite %s!\n\n", sat);
    fprintf( stderr, "   EXIT!\n\n");
    exit (-1);
  }
*/
  ...
  switch (chtype) {
	...
	default:
    		fprintf( stderr, "\n   error in channel params %d instead of [H-L]\n", chtype);
    		fprintf( stderr, "   EXIT!\n\n");
    		exit (-1);
  ...
  FILE * fraw = fopen(argv[8],"rb");

  if (fraw == NULL) {
    		fprintf( stderr, "\n Unable to find input file %s", argv[8]);
    		fprintf( stderr, "   EXIT!\n\n");
    		exit (-1);
  }
