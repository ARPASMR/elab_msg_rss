...
bool CWTDecoder::DecodeBufferBlock (const unsigned int i_Bs)
{
	...
	for (bY=0 ; bY<nbH ; bY++)
	{
		...
		for (bX=0 ; bX<nbW ; bX++)
		{
			...
			// Decode the stream
			if (! decoder.Decode (wblk, m_Param.m_nWTlevels, m_Param.m_nLossyBitPlanes) 
				|| ad.IsMarkerReached())
			{
			...
			if (m_Param.m_RestartInterval && nbBlock == m_Param.m_RestartInterval)
			{
				...
				markerNum++;
				if (bX < (nbW - 1) || bY < (nbH - 1)) ad.Start ();
				else bACStopped = true;
...
void CWTDecoder::DecodeBuffer (void)
{
	...
	m_Buf.real_seek (4);
	if (m_Param.m_BitsPerPixel == 0) m_Param.m_BitsPerPixel = 16;
