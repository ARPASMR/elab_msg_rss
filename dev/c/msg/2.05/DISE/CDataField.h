...
CDataField (void*	i_Ptr,		// Address of an allocated memory area.
    ...
    : Util::CSmartPtr<unsigned char>(static_cast<unsigned char*>(i_Ptr))
    , m_Size     (i_Size)
    , m_Allocated(i_Size)
    {}
 ...
 **/
 CDataField
     ...
     : Util::CSmartPtr<unsigned char>(i_DataField)
 ...
 CDataField(const unsigned __int64	i_Size       = 0,		// Size [bits] of new data field.
     ...
     : Util::CSmartPtr<unsigned char>(i_Size ? new unsigned char[(size_t)((i_Size + 7) / 8)]: NULL)
     , m_Size(i_Size)
     , m_Allocated(i_Size)
     {
	 try
...
// INTERFACES:
	...
	: CDataField(i_DataFieldUI)
	, m_NB(i_DataFieldUI.m_NB)
	, m_NC(i_DataFieldUI.m_NC)
	, m_NL(i_DataFieldUI.m_NL)
	, m_NR(i_DataFieldUI.m_NR)
	{
	}
	...
	: CDataField((unsigned __int64)i_NC * 
				 (unsigned __int64)i_NL *
				 (unsigned __int64)i_NR  , i_Initialize)
	, m_NB(i_NB)
	, m_NC(i_NC)
	, m_NL(i_NL)
	, m_NR(i_NR)
...
// INTERFACES:
	...
	: CDataField(i_DataField		)
	, m_NB		(i_DataField.GetNB())
	, m_NC		(i_DataField.GetNC())
	, m_NL		(i_DataField.GetNL())
	{
	}
	...
	: CDataField(i_Size, i_Initialise)
	, m_NB(i_NB)
	, m_NC(i_NC)
	, m_NL(i_NL)
	{
